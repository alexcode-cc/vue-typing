...
### AngularJS Git Commit Message Conventions (繁體中文)

所有 commit 訊息必須遵循 AngularJS 規範，並使用繁體中文：
...
| `fix` | 修復 bug |
| `docs` | 文件變更 |
| `style` | 程式碼格式（不影響程式邏輯的變更） |
| `refactor` | 重構（不是新增功能，也不是修復 bug） |
| `perf` | 效能優化 |
| `test` | 新增測試或變更現有測試 |
| `chore` | 建構流程或輔助工具的變更 |
...
```
fix(計分邏輯): 修正 WPM 計算錯誤

修正使用空白符號計算單字數的邏輯，
確保 WPM 計算結果正確。

fix(儲存邏輯): 修復 localStorage 範圍溢出問題
